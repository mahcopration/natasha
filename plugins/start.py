...
from helpers import b64_to_str, str_to_b64, get_current_time, shorten_url, shorten_urll
...
async def start_command(client: Client, message: Message):
    ...
    if message.text.startswith("/start token_"):
        ...
        try:
            ...
            if int(ad_msg.split(":")[1]) < get_current_time():
                ...
                return
            if int(ad_msg.split(":")[1]) > int(get_current_time() + 86500):
                await client.send_message(
                    message.chat.id,
    ...
    if uid not in ADMINS:
        ...
        if result is None:
            ...
            ad_code = str_to_b64(f"{uid}:{str(get_current_time() + 86500)}")
            ad_urll = shorten_urll(f"https://telegram.me/{TG_BOT_USERNAME}?start=token_{ad_code}")
